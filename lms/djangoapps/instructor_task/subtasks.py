...
def _generate_items_for_subtask(item_queryset, item_fields, total_num_items, total_num_subtasks, items_per_query, items_per_task):
    ...
    # We expect the chunking to be properly summing to the original count. But
    # because queueing does not happen in one transaction this may not always be true.
    if num_items_queued != total_num_items:
        info_msg = "Number of items generated by chunking {} not equal to original total {}".format(num_items_queued, total_num_items)
        TASK_LOG.info(info_msg)
